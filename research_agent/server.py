...
prompt_refine_messages = [{
    "role": "system",
    "content": "You are a research prompt assistant providing tool call response."
    }
]
def refine_prompt(
    messages,
    ...
    """
    # refine the research prompt using this tool
    tools = [{
        'type': 'function',
        'function': {
...
def search_and_reflect(
    ...
    research_messages.append({
        ...
        "content": (
            ...
            "and provide a refined search to get better research. The new refined search "
            "should be more focused and not more than 20 words. "
            )
        })
    completion = client.chat.completions.create(
        model="gpt-4",
    ...
    reflected_search_prompt = completion.choices[0].message.content
...
def main(port: int, log_level: str, json_response: bool) -> int:
    ...
    async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
        ...
        confirmed, refined_prompt = refine_prompt(
        ...
        research_messages.append ({
            "role": "user",
            "content": f"This is the research prompt: {reflected_search_prompt}"
        })
